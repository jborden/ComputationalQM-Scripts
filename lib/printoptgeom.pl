sub printoptgeom() 
{
	...
	if ($_[0] eq '')
	{
		print "printoptgeom() Error: No hash reference given\n";
		return 0;
	}
	else
	{
	...
	if ($_[1] eq '')
	{
		print "printoptgeo() Error: No dir given to write to!\n";
		return 0;
	}
	else 
	{
	...
	if ( $userfile eq "" )
	{
		...
		$userfilelength = 2;
		print "printoptgeom(): No optimized userfile given, defaulting to HF STO-G\n";
	}
	else 
	{
	...
	if ($lastgeom)
	{
		$geomcounter = $$geomdata{opt}{num};
	}
	#loop through each geometry until the last one
	while ($geomcounter <= $$geomdata{opt}{num})
	{
		...
		if ($line2param eq "")
		{
			...
			$newfile = join "",$dir,"/","Opt-",$methods[2],"-",$methods[4],"-",$line1param,"-",$$geomdata{opt}{$geomcounter}{$line1param},".com";
		}	
		#same like above, but for two parameters
		else
		{
			
			$newfile = join "",$dir,"/","Opt-",$methods[2],"-",$methods[4],"-",$line1param,"-",$$geomdata{opt}{$geomcounter}{$line1param},"-",$line2param,"-",$$geomdata{opt}{$geomcounter}{$line2param},".com";
		}
		#now to declare the chkpoint files so that they are unique
		#to the comfile
		...
		if ($line2param eq "")
		{
			$chkfile = join "","\%chk=/tmp/",$methods[2],"-",$methods[4],"-",$$geomdata{opt}{$geomcounter}{$line1param},".chk\n";
		}
		else 
		{

			$chkfile = join "","\%chk=/tmp/",$methods[2],"-",$methods[4],"-",$$geomdata{opt}{$geomcounter}{$line1param},"-",$$geomdata{opt}{$geomcounter}{$line2param},".chk\n";
		}
		...
		print COMFILE "$line1param is $$geomdata{opt}{$geomcounter}{$line1param}\n";
		if ($line2param ne "")
		{
			print COMFILE "$line2param is $$geomdata{opt}{$geomcounter}{$line2param}\n";
		}
		print COMFILE "SCF Done: $$geomdata{opt}{$geomcounter}{scfgas}\n";	
		print COMFILE "\n";
...
.
	...
	my $atomcounter = 1;
		while ($atomcounter <= $$geomdata{natoms})
		{
			...
			$type = $$geomdata{opt}{$geomcounter}{$atomcounter}{type};
			$x = $$geomdata{opt}{$geomcounter}{$atomcounter}{x};
			$y = $$geomdata{opt}{$geomcounter}{$atomcounter}{y};
			$z = $$geomdata{opt}{$geomcounter}{$atomcounter}{z};
			write COMFILE;
			$atomcounter++;
		}
	$filecounter = 2;
