...
# $value   = $$zmat{nonopt}{vars}{$geomcounter}{$$zmat{vars}{initial}{$i}}
...
$constant,$value
.
# $value = $$zmat{nonopt}{vars}{$geomcounter}{$newconstant}
	...
	if ($_[0] eq '')
	{
		print "printnonoptzmat() Error: No hash array given\n";
		return 0;
	}
	else
	{
	...
	if ($_[1] eq '')
	{
		print "printnonoptzmat() Error: No dir specified\n";
		return 0;
	}
	else
	{
	...
	{
	        open(USERFILE,"<$nonoptuserfile") or die "Could not open $nonoptuserfile\n";
	        @Userfile = <USERFILE>;
	        close USERFILE;
	...
	if ($lastgeom)
	{
		$geomcounter = $$zmat{nonopt}{num};
	}
	#go through each nonopt geoms until the last is reached
	while ($geomcounter <= $$zmat{nonopt}{num})
	{
		#properly format the user specified line parameters
		$$zmat{nonopt}{vars}{$geomcounter}{$line1param} = sprintf("%.2f",$$zmat{nonopt}{vars}{$geomcounter}{$line1param});
		
		$$zmat{nonopt}{vars}{$geomcounter}{$line2param} = sprintf("%.2f",$$zmat{nonopt}{vars}{$geomcounter}{$line2param});
		#create a new description to create a com file from	
		if ($line2param eq '')
		{
		$newdescription = join "",$methods[2],"-",$methods[4],"-non-optimized-zmatrix-","$line1param-","$$zmat{nonopt}{vars}{$geomcounter}{$line1param}";
		}
		else
		{
		$newdescription = join "",$methods[2],"-",$methods[4],"-non-optimized-zmatrix-","$line1param-","$$zmat{nonopt}{vars}{$geomcounter}{$line1param}-","$line2param-","$$zmat{nonopt}{vars}{$geomcounter}{$line2param}";
		}
		...
		print COMFILE "$line1param is $$zmat{nonopt}{vars}{$geomcounter}{$line1param}\n";
		if ($line2param ne '')
		{
		print COMFILE "$line2param is $$zmat{nonopt}{vars}{$geomcounter}{$line2param}\n";
		}
		#write the SCF gas related to the selected geometries
		#to the com file
		print COMFILE "SCF Done: $$zmat{nonopt}{$geomcounter}{scfgas}\n";
		print COMFILE "\n";
...
#		}	
		...
		{
			if ($$zmat{initial}{variable}{$paramcount} eq $line1param)
			{
				$paramcount++;
				next;
			}
			if ($$zmat{initial}{variable}{$paramcount} eq $line2param)
			{
				$paramcount++;
				next;
			}
			...
			$value   = $$zmat{nonopt}{vars}{$geomcounter}{$$zmat{initial}{variable}{$paramcount}};
			write COMFILE;
			$paramcount++;
		}
...
#					$value = $$zmat{nonopt}{vars}{$geomcounter}{$$zmat{constants}{variable}{$constantcount}};
			#select STDOUT;
#					write COMFILE;
#					$constantcount++;
#					next;
...
#			}
			...
			print COMFILE "$line1param $$zmat{nonopt}{vars}{$geomcounter}{$line1param}\n";
			if ($line2param ne '')
			{
				print COMFILE "$line2param $$zmat{nonopt}{vars}{$geomcounter}{$line2param}\n";
			}
